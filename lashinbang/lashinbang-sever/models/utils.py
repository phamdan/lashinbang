...
def resize_prop_rect(src, size_image=720):
...
def convert_image_to_tensor(image, device):
    ...
    if image is None:
        return None, None, None
    w, h = image.shape[1], image.shape[0]
    image, s = resize_prop_rect(image)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    w_new, h_new = image.shape[1], image.shape[0]
    scales = (float(w) / float(w_new), float(h) / float(h_new))
    inp = frame2tensor(gray, device)
...
def convert_img_to_tensor(image, device):
    if image is None:
        return None
    inp = frame2tensor(image, device)
    return inp
...
def make_draw_matches(image0, image1, kpts0, kpts1, mkpts0,
                            mkpts1, color, text,
                            show_keypoints=False, margin=10,
                            small_text=[]):
    H0, W0 = image0.shape[:2]
